...
public class AnimationHandler {
	...
	public AnimationHandler (Sprite image, double frameTime) {
		this.image = image;
		startTime = MainLoop.getStartTime();
		startFrame = 0;
	...
	public void draw (double x, double y) {
		...
		if (image != null) {
			if (frameTime == 0) {
				...
				long elapsedTime = MainLoop.getStartTime() - startTime;
	...
	/**
	 * allwyas draws the first frame becasue I am being a bit lazy right now
	 * 
	 * also doensent use cutoff widths or heights for the same reason
	 */
	public void draw (double x, double y, Sprite toDraw) {
		if (visible) {
		if (toDraw != null) {
			if (frameTime == 0) {
				toDraw.draw ((int)x, (int)y, flipHorizontal, flipVertical,0);
				startTime = MainLoop.getStartTime();
			} else {
				long elapsedTime = MainLoop.getStartTime() - startTime;
				int elapsedFrames = ((int)(((double)elapsedTime) / ((double)frameTime)) + startFrame);
	...
	//lots of code copy pasted from stack overflow here note don't use better options exist
	...
	public void playFrom (int where, int too) {
		...
		this.startTime = MainLoop.getStartTime();
	}
