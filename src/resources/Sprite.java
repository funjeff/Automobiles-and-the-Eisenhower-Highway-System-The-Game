...
public class Sprite {
	...
	public Sprite (String imagepath, int gridX, int gridY) {
		this(imagepath, new SpriteParser ("grid " + gridX + " " + gridY, 0));
	}
	/**
	...
	public void draw (double usedX, double usedY, int frame, AffineTransform transform) {
			Graphics2D windowGraphics = (Graphics2D)MainLoop.getWindow().getBuffer();
	...
	public void draw (int usedX, int usedY, int frame) {
		if (frame < images.length) {
			MainLoop.getWindow().getBuffer().drawImage (images [frame], usedX, usedY, null);
		}
	}
	/**
	...
	public void draw (int usedX, int usedY, int frame, int width,int height) {
		if (frame < images.length) {
			MainLoop.getWindow().getBuffer().drawImage (	images[frame].getSubimage(0, 0, width, height), usedX, usedY, null);
		}
	}
	/**
	 * Draws the given frame of this sprite at the given x and y coordinates.
	...
	public void draw (int usedX, int usedY, boolean flipHorizontal, boolean flipVertical, int frame) {
		...
		if (frame < images.length) {
			MainLoop.getWindow().getBuffer().drawImage (getFrame (frame), usedX, usedY, usedX + getFrame (frame).getWidth (), usedY + getFrame (frame).getHeight (), x1, y1, x2, y2, null);
		}
	}
	/**
	 * Draws the given frame of this sprite at the given x and y coordinates.
	...
	public void draw (int usedX, int usedY, boolean flipHorizontal, boolean flipVertical, int frame, int width, int height) {
		...
		if (frame < images.length) {
			MainLoop.getWindow().getBuffer().drawImage (getFrame (frame).getSubimage(0, 0, width, height), usedX, usedY, usedX + getFrame(frame).getWidth(), usedY + + getFrame(frame).getHeight(), x1, y1, x2, y2, null);
		}
	}
	/**
